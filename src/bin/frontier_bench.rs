...
async fn setup_test_model() -> (Arc<Database>, std::path::PathBuf) {
    let db = Arc::new(Database::new(None, 1536));
    let temp_dir = std::env::temp_dir().join("bramha_frontier_bench_test");
    let _ = std::fs::remove_dir_all(&temp_dir);
    std::fs::create_dir_all(&temp_dir).unwrap();

    // Write a test tokenizer
    let test_tokenizer = r#"{"version":"1.0","truncation":null,"padding":null,"added_tokens":[],"normalizer":null,"pre_tokenizer":null,"post_processor":null,"decoder":null,"model":{"type":"BPE","vocab":{"<s>":0,"</s>":1,"<unk>":2,"hi":3,"hello":4,"world":5},"merges":[]}}"#;
    std::fs::write(temp_dir.join("tokenizer.json"), test_tokenizer).unwrap();
...

    {
        ...
        tensor_guard.restore_model_at_path("frontier-test-model".to_string(), &temp_dir);
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (db, temp_dir) = rt.block_on(setup_test_model());
