...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !model_path.exists() {
        println!("❌ Model path {} not found. Falling back to tinyllama for shadow test.", model_path.display());
    }
    ...
    println!("Running golden dataset shadow scan against {}...", active_model);
    ...
    if let Some(model) = models.get_mut(active_model) {
        ...
        if let Some(page) = model.layers.get(&tensor_name) {
            println!("✅ Successfully loaded tensor page! Byte length: {}", page.as_bytes().len());
            ...
            let hidden_dim = if active_model.contains("qwen") { 896 } else { 2048 };
            let intermediate_dim = weights.len() / hidden_dim;
            
            println!("Simulating input activation vector of size {}...", intermediate_dim);
            let x: Vec<f32> = (0..intermediate_dim).map(|i| (i as f32 % 10.0) / 10.0).collect();
            
            println!("Running 2:4 Sparse MatMul prediction...");
            let sparse_out = bramha::inference::sparse_predictor::sparse_matvec_mul_2_4(&x, weights, intermediate_dim);
            ...
            let similarity = bramha::inference::sparse_predictor::cosine_similarity(&dense_out, &sparse_out);
            println!("=====================================================");
            println!("📊 SPARSE PREDICTOR ACCURACY (Cosine Similarity): {:.4}", similarity);
            println!("=====================================================");
        ...
        println!("🧹 Unloaded {} from physical memory to prevent OOM.", tensor_name);
    }
