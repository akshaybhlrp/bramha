...
fn bench_model_load_time(c: &mut Criterion) {
    ...
    let db = Arc::new(bramha::storage::Database::new(Some("storage/benchmark_db".to_string()), 1024));
...
fn bench_first_token_latency(c: &mut Criterion) {
    ...
    c.bench_function("first_token_2k_cold", |b| {
        b.iter(|| {
            rt.block_on(async {
                let res = bramha::inference::engine::InferenceEngine::new(None).generate(
                    db.clone(),
                    "tinyllama",
                    "Hello world",
                    1,
...
criterion_group!(benches, bench_model_load_time, bench_first_token_latency, bench_sustained_throughput);
