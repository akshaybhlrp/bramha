...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result_cpu = InferenceEngine::new(None).generate(
        db.clone(),
        "tinyllama-1.1b",
        prompt,
    ...
    bramha::inference::set_cpu_only(false);
    {
        ...
        let mut cache = bramha::inference::engine::VramCache::global().lock().unwrap();
        cache.max_vram_bytes = None;
    }
    
    let result_gpu = InferenceEngine::new(None).generate(
        db.clone(),
        "tinyllama-1.1b",
        prompt,
        25, // generate 25 tokens
