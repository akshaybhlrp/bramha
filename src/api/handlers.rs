...
use crate::middleware::auth::{RequireReadOnly, RequireWrite, RequireAdmin};
...
pub async fn get_stats(State(db): State<SharedState>) -> Json<StatsResponse> {
    ...
    let degraded_collections: Vec<String> = state.collections.values()
...
pub async fn create_collection(
    ...
    if state.collections.contains_key(&payload.name) {
        return Err((StatusCode::BAD_REQUEST, "Collection already exists".to_string()));
    }
...
pub async fn list_collections(State(db): State<SharedState>) -> Json<Vec<CollectionInfo>> {
    ...
    let infos = state.collections
        ...
        .map(|c| CollectionInfo {
            ...
            davies_bouldin_index: c.index.as_ref().and_then(|idx| idx.analytics.as_ref().map(|a| a.davies_bouldin_index)),
            silhouette_score: c.index.as_ref().and_then(|idx| idx.analytics.as_ref().map(|a| a.silhouette_score)),
            imbalance_ratio: c.index.as_ref().and_then(|idx| idx.analytics.as_ref().map(|a| a.imbalance_ratio)),
        })
        .collect();
    Json(infos)
}
...
pub async fn get_collection(
    ...
    let state = db.state.read().await;
    let c = state.collections.get(&name)
    ...
    if state.collections.remove(&name).is_some() {
        ...
        tokio::spawn(async move {
            if is_preserved {
                ...
                println!("🔄 Recreating deleted preserved collection '{}' automatically in the background...", name_clone);
                let mut collection = Collection::new(name_clone.clone(), 384, Metric::Cosine);
                if let Err(e) = collection.init_sqlite_index() {
                    println!("⚠️ Failed to rebuild SQLite index for recreated collection '{}': {}", name_clone, e);
                }
...
pub async fn upsert_vectors(
    ...
    let mut state = db.state.write().await;
    let c = state.collections.get_mut(&name)
        .ok_or((StatusCode::NOT_FOUND, "Collection not found".to_string()))?;
...
pub async fn query_collection(
    ...
    let results = if payload.hybrid.unwrap_or(false) && payload.query_text.is_some() {
        ...
        c.hybrid_search(text, &payload.vector, payload.k, payload.filter.as_ref(), payload.use_index)
    } else {
        c.search(&payload.vector, payload.k, payload.filter.as_ref(), payload.use_index)
    };
...
pub async fn delete_vectors(
    ...
    Ok(Json(format!("Successfully deleted {} vectors", deleted_count)))
}
...
pub async fn reindex_collection(
    ...
    let index_type = payload.index_type.clone().unwrap_or_else(|| "ivf".to_string());
    if index_type == "hnsw" {
...
pub async fn llm_embed(
    ...
    match crate::inference::embedder::Embedder::get_global().await {
        Ok(embedder) => {
            match embedder.embed(&payload.text) {
                Ok(embedding) => {
                ...
                Err(e) => {
                    println!("⚠️ Native embedder error: {}. Falling back to sidecar...", e);
                }
            }
        }
        Err(e) => {
            println!("⚠️ Failed to initialize native embedder: {}. Falling back to sidecar...", e);
        }
    }
    ...
    let resp = client
        ...
        .map_err(|e| {
            (
                ...
                format!("Failed to reach AirLLM sidecar: {}. Is airllm_server.py running on port 8001?", e),
            )
        })?;

    let sidecar_resp: SidecarEmbedResponse = resp
        .json()
        .await
        .map_err(|e| (StatusCode::BAD_GATEWAY, format!("Invalid sidecar response: {}", e)))?;
...
async fn fetch_sidecar_embedding(text: &str) -> Result<Vec<f32>, (StatusCode, String)> {
    ...
    let embed_data: SidecarEmbedResponse = embed_resp
        .json()
        .await
        .map_err(|e| (StatusCode::BAD_GATEWAY, format!("Invalid embed response: {}", e)))?;
...
pub async fn llm_rag(
    ...
    let log_msg = format!("🔍 [RAG Pipeline] Received prompt: \"{}\" targeting collection: \"{}\"", payload.prompt, payload.collection_name);
    ...
    let query_vector = match crate::inference::embedder::Embedder::get_global().await {
        Ok(embedder) => {
            match embedder.embed(&payload.prompt) {
                Ok(embedding) => embedding,
                Err(e) => {
                    let log_msg = format!("⚠️ Native embedder error in RAG: {}. Falling back to sidecar...", e);
                    crate::inference::engine::InferenceLogger::global().record_log(log_msg);
        ...
        Err(e) => {
            let log_msg = format!("⚠️ Failed to initialize native embedder in RAG: {}. Falling back to sidecar...", e);
            crate::inference::engine::InferenceLogger::global().record_log(log_msg);
            fetch_sidecar_embedding(&payload.prompt).await?
        }
    };

    let strategy = payload.strategy.clone().unwrap_or_else(|| "standard".to_string());
    let model_name = payload.model_name.clone().unwrap_or_else(|| "tinyllama".to_string());
    ...
    if payload.use_cache {
        ...
        if let Some((cached_completion, score)) = state.cache.lookup(&query_vector) {
            ...
            let log_msg = format!("✨ [Semantic Cache Hit] Retrieving answer from cache (sub-ms). Similarity: {:.4}", score);
            crate::inference::engine::InferenceLogger::global().record_log(log_msg);
    ...
    let log_msg = format!("⚡ [Semantic Cache Miss] Running database query (strategy: \"{}\", hybrid: {})...", strategy, payload.hybrid.unwrap_or(false) || strategy == "hybrid");
    crate::inference::engine::InferenceLogger::global().record_log(log_msg);
    ...
    let search_results = match strategy.as_str() {
        "multi_query" => {
            crate::index::strategies::RetrievalStrategies::multi_query_search(
                db.clone(),
        ...
        "hyde" => {
            crate::index::strategies::RetrievalStrategies::hyde_search(
                db.clone(),
                &payload.collection_name,
                &model_name,
                &payload.prompt,
        ...
        _ => {
            ...
            if payload.hybrid.unwrap_or(false) || strategy == "hybrid" {
                collection.hybrid_search(&payload.prompt, &query_vector, payload.k, None, payload.use_index)
            } else {
    ...
    let log_msg = format!("📄 Retrieved {} context chunks from database.", context_chunks.len());
    crate::inference::engine::InferenceLogger::global().record_log(log_msg);
    ...
    if payload.use_rerank.unwrap_or(false) && !context_chunks.is_empty() {
        match crate::inference::reranker::Reranker::get_global().await {
            Ok(reranker) => {
                let log_msg = format!("🎯 Running native WGPU Cross-Encoder reranking on {} retrieved chunks...", context_chunks.len());
                crate::inference::engine::InferenceLogger::global().record_log(log_msg);
                for chunk in &mut context_chunks {
                    let doc_text = chunk.metadata.get("text")
                ...
                context_chunks.sort_by(|a, b| b.score.partial_cmp(&a.score).unwrap_or(std::cmp::Ordering::Equal));
                
                // Truncate to top rerank_k results
                let target_k = payload.rerank_k.unwrap_or(3);
                context_chunks.truncate(target_k);
                let log_msg = format!("🎯 Reranking complete. Selected top-{} chunks.", context_chunks.len());
                crate::inference::engine::InferenceLogger::global().record_log(log_msg);
            }
            Err(e) => {
                let log_msg = format!("⚠️ Failed to initialize native cross-encoder reranker: {}. Proceeding without reranking...", e);
                crate::inference::engine::InferenceLogger::global().record_log(log_msg);
            }
        }
    }

    {
        ...
        for (i, chunk) in context_chunks.iter().enumerate() {
            ...
            if display_text.is_empty() {
                ...
                display_text = chunk.metadata.get("text")
                    .or_else(|| chunk.metadata.get("content"))
                    .and_then(|v| v.as_str())
                    .unwrap_or_else(|| "")
                    .to_string();
        ...
        if payload.compress_context.unwrap_or(false) {
            ...
            if hybrid_alpha < 0.4 {
                let log_msg = format!("🛡️ Skipping context compression due to highly lexical query (alpha={:.2}) to preserve exact grounding.", hybrid_alpha);
                crate::inference::engine::InferenceLogger::global().record_log(log_msg);
            } else {
                let log_msg = "🗜️ Compressing context chunks to optimize LLM reasoning window...".to_string();
                crate::inference::engine::InferenceLogger::global().record_log(log_msg);
                ...
                if let Ok(comp_res) = db.inference_queue.submit(
                    model_name.clone(),
                    compression_prompt,
    ...
    let result = match db.inference_queue.submit(
        model_name.clone(),
        ...
        Err(e) => {
            let log_msg = format!("⚠️ Local in-process inference failed: {}. Falling back to sidecar...", e);
            crate::inference::engine::InferenceLogger::global().record_log(log_msg);
            let gen_resp = client
            ...
            let gen_data: SidecarGenerateResponse = gen_resp
                .json()
                .await
                .map_err(|err| (StatusCode::BAD_GATEWAY, format!("Invalid generate response: {}", err)))?;
...
pub async fn llm_embed_batch(
    ...
    let sidecar_resp: serde_json::Value = resp
        .json()
        .await
        .map_err(|e| (StatusCode::BAD_GATEWAY, format!("Invalid sidecar response: {}", e)))?;

    let embeddings: Vec<Vec<f32>> = serde_json::from_value(sidecar_resp["embeddings"].clone())
        .map_err(|e| (StatusCode::BAD_GATEWAY, format!("Invalid sidecar embeddings format: {}", e)))?;
    let dimensions = sidecar_resp["dimensions"].as_u64().unwrap_or(2048) as usize;
    let model = sidecar_resp["model"].as_str().unwrap_or("unknown").to_string();
...
pub async fn llm_load_model(
    ...
    if let Some(limit) = payload.resource_limit {
        let mut cache = crate::inference::engine::VramCache::global().lock().unwrap();
    ...
    // 1. Lock and update in active tensor_db models
    {
        ...
        if let Some(ref name) = resolved_name {
            if let Some(model) = tensor_db.models.get_mut(name) {
                ...
                println!("🧠 Resolved model '{}' active_device updated to '{}' (CPU_ONLY={})", name, target_device, is_cpu);
            }
        }
    }
    ...
    if let Some(ref name) = resolved_name {
        ...
        if let Some(meta) = state_guard.model_registry.get_mut(name) {
            ...
            println!("💾 Persisted model_registry for '{}' active_device: '{}'", name, target_device);
        }
    }
...
pub async fn llm_logs(
    ...
    if let Ok(resp) = client.get(format!("{}/logs?since={}", SIDECAR_URL, since)).send().await {
    ...
    let logs = if native_logs.is_empty() && since == 0 {
        vec![
            crate::inference::engine::LogEntry {
                message: "[Bramha Startup] In-process BERT MiniLM embedder initialized successfully.".to_string(),
...
pub async fn get_model_layers(
    ...
    let model = tensor_db.models.get(&model_name).ok_or_else(|| {
        (StatusCode::NOT_FOUND, format!("Model '{}' not found", model_name))
    })?;
    
    // Return all layer names
    let layers: Vec<String> = model.layers.keys().cloned().collect();
    Ok(Json(serde_json::json!({ "model": model_name, "layer_count": layers.len(), "layers": layers })))
}
...
pub async fn ingest_model(
    ...
    // Register task
    {
        ...
        db_state.ingestion_tasks.insert(task_id.clone(), "Processing: Starting ingestion...".to_string());
    }
    ...
    tokio::spawn(async move {
        ...
        let base_path = {
            ...
            tensor_db.models.get(&model_name_clone).unwrap().base_path.clone()
        };

        // Update status
        {
            let mut db_state = state_clone.state.write().await;
            db_state.ingestion_tasks.insert(task_id_clone.clone(), format!("Processing: Scanning safetensors in {}...", payload_path));
        }
        ...
        let shard_result = tokio::task::spawn_blocking(move || -> Result<crate::storage::tensor_db::ModelTable, String> {
            let mut temp_table = crate::storage::tensor_db::ModelTable::new(model_name_inner.clone(), base_path);
            let target_path = std::path::Path::new(&payload_path_inner);
            match find_safetensors_files(target_path) {
                Ok(files) => {
                    if files.is_empty() {
                        Err(format!("No .safetensors files found in {}", payload_path_inner))
                    } else {
                        for file in files {
                            println!("Ingesting safetensors shard: {:?}", file);
                            if let Err(e) = crate::storage::safetensors_loader::shard_safetensors_file(&mut temp_table, &file) {
            ...
            Ok(temp_table) => {
                ...
                let col_name = model_name_clone.clone();
                {
                    ...
                    if !db_state.collections.contains_key(&col_name) {
                        println!("🔄 Creating new collection '{}' for ingested model '{}'...", col_name, model_name_clone);
                        let mut collection = Collection::new(col_name.clone(), 384, Metric::Cosine);
                        if let Err(e) = collection.init_sqlite_index() {
                            println!("⚠️ Failed to rebuild SQLite index for ingested model's collection '{}': {}", col_name, e);
                        }
                        db_state.collections.insert(col_name.clone(), collection);
                    }
                    db_state.preserved_collections.insert(model_name_clone.clone(), col_name.clone());
                    ...
                    db_state.model_registry.insert(model_name_clone.clone(), metadata);
                }
                ...
                {
                    let mut db_state = state_clone.state.write().await;
                    db_state.ingestion_tasks.insert(task_id_clone, "Completed".to_string());
                }
            }
            Err(e) => {
                let mut db_state = state_clone.state.write().await;
                db_state.ingestion_tasks.insert(task_id_clone, format!("Failed: {}", e));
            }
        }
    });
...
pub async fn delete_model(
    ...
    if tensor_db.models.remove(&model_name).is_some() {
        ...
        Ok(Json(serde_json::json!({ "status": "success", "message": format!("Deleted model {}", model_name) })))
    } else {
        Err((StatusCode::NOT_FOUND, format!("Model '{}' not found", model_name)))
    }
}
...
pub async fn fetch_tensor_layer(
    ...
    let layer_bytes = model.fetch_layer(&layer_id).ok_or_else(|| {
        (StatusCode::NOT_FOUND, format!("Layer '{}' not found in model '{}'", layer_id, model_name))
    })?;
...
pub async fn build_model_index(
    ...
    let model_view = crate::storage::model_view::ModelView::load(&model_view_path)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to load model_view: {}", e)))?;

    let content_dir = base_path.parent().unwrap_or(std::path::Path::new("")).join("content");
    let block_db = crate::storage::block_db::BlockDB::new(&content_dir)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to load block_db: {}", e)))?;

    let layer_indices_dir = base_path.join("layer_indices");
    std::fs::create_dir_all(&layer_indices_dir)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to create layer_indices dir: {}", e)))?;
...
pub async fn update_tensor_settings(
    ...
    if let Err(e) = std::fs::create_dir_all(&path) {
        return Err((StatusCode::BAD_REQUEST, format!("Invalid directory path: {}", e)));
    }
    ...
    if let Err(e) = state.save().await {
        return Err((StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to save state: {}", e)));
    }
...
pub async fn generate_text(
    ...
    if device == "cpu" {
        ...
        unsafe { std::env::set_var("SPANDA_OVERRIDE", "dense"); }
    } else if device == "sparse" {
        crate::inference::set_cpu_only(false);
        unsafe { std::env::set_var("SPANDA_OVERRIDE", "sparse"); }
    } else {
        crate::inference::set_cpu_only(false);
        unsafe { std::env::set_var("SPANDA_OVERRIDE", "dense"); }
    }
    ...
    let start_time = std::time::Instant::now();
    let result = state.inference_queue.submit(
...
pub async fn retrieve_kv_cache_handler(
    ...
    let manager = crate::storage::cache_db::KvCacheManager::default();
    let entry_opt = manager.retrieve(&payload.session_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
...
pub async fn clear_kv_cache_handler() -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let manager = crate::storage::cache_db::KvCacheManager::default();
    manager.clear().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
...
pub async fn benchmark_quantization(
    ...
    let _ = state.inference_queue.submit(
        payload.model_name.clone(),
        payload.prompt.clone(),
        5,
        0.0,
...
pub async fn list_sessions() -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    ...
    let session_list: Vec<crate::storage::session_store::Session> = sessions.into_values().collect();
...
pub async fn upsert_session(
    ...
    let store = crate::storage::session_store::SessionStore::new();
    store.upsert(session.clone()).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(serde_json::json!({
        "status": "success",
