...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bramha::storage::tensor_db::TensorDB { models, block_db, .. } = &mut *tensor_db_write;
    ...
    if let Some(model) = models.get_mut("Llama") {
        ...
        if let Some(page) = model.layers.get(tensor_name) {
            ...
            println!("Loaded tensor '{}': shape={:?}, dtype={:?}, bytes_len={}", 
                tensor_name, page.shape, page.dtype, bytes.len());
