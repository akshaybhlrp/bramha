...
impl GoalGraph {
    ...
    pub fn new(prompt: &str, max_hops: usize) -> Self {
        ...
        let is_comparative = prompt.to_lowercase().contains("compare")
            || prompt.to_lowercase().contains("difference");
        let is_multi_part = prompt.to_lowercase().contains("first")
            || prompt.to_lowercase().contains("then")
            || prompt.contains(';');
        ...
        if is_comparative {
            ...
            tasks.push(SubTask {
                ...
                description:
                    "Analyze the second target entity and outline similarities/differences"
                        .to_string(),
                query: Some(prompt.to_string()),
                status: TaskStatus::Pending,
                result: String::new(),
            ...
            tasks.push(SubTask {
                id: "subtask_2".to_string(),
                description: "Extend context to compile secondary insights and dependencies"
                    .to_string(),
                query: Some(prompt.to_string()),
                status: TaskStatus::Pending,
                result: String::new(),
            ...
            tasks.push(SubTask {
                ...
                description: "Execute standard direct semantic retrieval and answer synthesis"
                    .to_string(),
                query: Some(prompt.to_string()),
                status: TaskStatus::Pending,
                result: String::new(),
...
mod tests {
    ...
    fn test_goal_graph_decomposition_and_execution() {
        ...
        let retrieve_fn =
            |_query: &str| -> Vec<String> { vec!["Rust has zero-cost abstractions".to_string()] };
    ...
    fn test_goal_graph_max_hops_bound() {
        ...
        let retrieve_fn =
            |_query: &str| -> Vec<String> { vec!["Rust is extremely fast".to_string()] };
