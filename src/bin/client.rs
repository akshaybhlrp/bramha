...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let url = format!("http://localhost:8000/api/tensor/{}/{}", model_name, layer_name);
    ...
    println!("   ✅ Fetched {} bytes in {:?}", raw_bytes.len(), fetch_time);
    ...
    println!("   ✅ Tensor loaded in memory: Shape {:?}", q_weight_tensor.shape());
    ...
    let mock_input_embedding = Tensor::<B, 2>::random(Shape::from([1, cols]), burn::tensor::Distribution::Normal(0.0, 1.0), &device);
    println!("   - Created incoming embedding: Shape {:?}", mock_input_embedding.shape());
